...
export default function DriverView() {
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text("Reporte de Pasajeros", 14, 15);
    ...
    doc.save("pasajeros_vista_actual.pdf");
  };
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[#2D1E2F] text-2xl">Pasajeros</h1>
          <p className="text-[#2D1E2F]/50 text-sm mt-1">
            {totalCount !== undefined ? totalCount : "Cargando"} pasajeros en
            total
          </p>
        </div>
            ...
            <tbody className="divide-y divide-[#2D1E2F]/8">
              {!loading && filtered.length === 0 ? (
                <tr>
                  ...
                  >
                    No se encontraron pasajeros en esta página
                  </td>
                </tr>
              ) : (
                filtered.map((user) => {
